...
class WaterService(StatisticService):
    ...
    def add_drunk_water(self, water_request: WaterRequestDTO) -> WaterResponseDTO:
        ...
        try:
            today = date.today()
            request = StatisticRequestDTO(user_id=water_request.user_id, statistic_date=today)
    ...
    def _wrap_func(self, result: dict) -> list[WaterResponseDTO]:
        return [WaterResponseDTO(day=key, drunk_water=data) for key, data in result.items()]
