...
class TestWaterDrunkService(BaseTestService):
    ...
    def test_failed_add_water(self):
        with pytest.raises(Exception) as e_info:
            self.water_service.add_drunk_water(
                WaterRequestDTO(user_id=None, drunk_water=250)
            )
    ...
    def test_add_water(self):
        drunk = self.water_service.add_drunk_water(
            WaterRequestDTO(user_id=self.user_id, drunk_water=250)
        )
    ...
    def test_add_water_3_times(self):
        ...
        assert (
                drunk.drunk_water
                == request_1.drunk_water + request_2.drunk_water + request_3.drunk_water
        )

    @pytest.fixture
    def initialize_2000(self):
        self.water_service.add_drunk_water(
            WaterRequestDTO(user_id=self.user_id, drunk_water=500)
        )
        self.water_service.add_drunk_water(
            WaterRequestDTO(user_id=self.user_id, drunk_water=1000)
        )
        self.water_service.add_drunk_water(
            WaterRequestDTO(user_id=self.user_id, drunk_water=500)
        )
