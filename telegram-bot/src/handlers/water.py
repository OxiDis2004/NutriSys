...
def format_answer(telegram_id: int, water: int) -> str:
    translated_text = translate(telegram_id, "You drunk total water:")
    if water >= 1000:
        liters_str = f"{water/1000:.2f}".rstrip("0").rstrip(".")
        return f"{translated_text} {liters_str} l"
    else:
        return f"{translated_text} {water} ml"
